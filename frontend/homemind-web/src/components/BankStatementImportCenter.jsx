...
export default function BankStatementImportCenter() {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      setStatus("קורא את קובץ הבנק...");

      const result = await parseBankStatementFile(file);

      addBankTransactions(result.transactions);

      const updatedSummary = calculateBankTransactionsSummary();
      setSummary(updatedSummary);

      setStatus(
        `יובאו בהצלחה ${result.transactionsCount} תנועות מתוך הקובץ ${file.name}`
      );
    } catch (error) {
      console.error(error);
